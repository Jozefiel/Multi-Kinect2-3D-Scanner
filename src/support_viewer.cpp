    ...
    while(snap_running)
    {
        for(auto connected_cams_number=0;connected_cams_number<connected_cams.size();connected_cams_number++)
        {
            if(cam_frames->at(connected_cams_number).size()>6)
            {
                ...
                {
                    emit newRangedRGBD(QPixmap::fromImage(QImage(cam_frames->at(connected_cams_number).back().rangedRGBDMat.data,cam_frames->at(connected_cams_number).back().rangedRGBDMat.cols,cam_frames->at(connected_cams_number).back().rangedRGBDMat.rows,cam_frames->at(connected_cams_number).back().rangedRGBDMat.step,QImage::Format_RGB888).rgbSwapped()),connected_cams_number);
                }
